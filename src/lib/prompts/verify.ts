...
### Step C: 사전 지식 환각 점검 (Phase G)
이 작품을 사전에 알고 있다면, 보고서가 **제공된 원문 범위를 벗어나** 다른 곳(작품의 유명한 결말, 책 전체 줄거리 등)에서 가져온 사실을 적었는지 확인하세요. 입력에 없는 사건·반전·인물 행동을 보고서가 단정적으로 서술했다면, 그 문장 단위로 \`section: "fabricated"\`, \`description\`에 어떤 사실이 입력에 없는지 명시, \`suggested_fix\`에 "이 문장을 삭제하거나 [추정]으로 표시" 권고를 적으세요.
